...
async function certificate({ address, rpc }: { address: string; rpc: string }): Promise<string> {
  const provider = new Web3(new Web3.providers.HttpProvider(rpc));
  const contract = new provider.eth.Contract(RegenerationCredit.abi, RegenerationCredit.address);

  const response = await contract.methods.certificate(address).call() as string;
  return response;
}
